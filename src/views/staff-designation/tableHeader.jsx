...
const TableHeader = ({ data, designationFilter, handleAddOpen, searchQuery, setSearchQuery }) => {
  const {
  ...
  useEffect(() => {
    if (isInitialLoad) {
      ...
      designationFilter &&
        designationFilter({
          designationName: { value: initialFilters.designationName, mode: filterModes.designationName },
  ...
  }, [isInitialLoad, designationFilter, filters, filterModes])
  ...
  const submitData = (data) => {
    const appliedFilters = {
      designationName: { value: data.designationName, mode: filterModes.designationName },
    ...
    designationFilter && designationFilter(appliedFilters)
    setFilters(data)
    handlePopoverClose()
  }
  ...
  const clearFilter = () => {
    ...
    designationFilter && designationFilter(null)
  }
  const clearSpecificFilter = (filterKey) => {
    setFilters((prevFilters) => {
      ...
      const appliedFilters = {
        designationName: { value: updatedFilters.designationName, mode: filterModes.designationName },
      ...
      if (designationFilter) {
        designationFilter(appliedFilters)
      }
  ...
  const filterFields = [
    { title: 'Name', value: 'designationName' },
            ...
            <Typography variant="buttonLabel" sx={addIconMainContainer}>
              {!isXs && isSmall && isScrolled && (
                <Box sx={addIconContainer}>
                  <CustomTooltip arrow title="Add Staff Designation" placement="top">
