...
import { Box, Grid, Typography, TextField, MenuItem, Select, InputAdornment } from '@mui/material';
import CustomTextField from '@/components/customTextField';
import formatPrice from '@/hooks/priceCommas';

const OrderPartPriceCalculation = ({
    orderParts,
    activeTab,
    setOrderParts,
    service,
    ...
    const calculateFileQuantity = useCallback((file) => {
        const isMediaFile = file?.orderPartFile?.mediaType?.startsWith('AUDIO') ||
        ...
        if (
            ...
            return Number(currentOrderPart?.unitQuantities && currentOrderPart?.unitQuantities[0]?.quantity ? currentOrderPart?.unitQuantities[0]?.quantity : 0).toFixed(2);
        }
    ...
    const calculateTotalPrice = () => {
        ...
        const filesTotal = currentOrderPart?.orderPartFiles?.reduce((total, file) => {
            return total + (parseFloat(file.totalPriceForFile || 0));
        }, 0) || 0;

        // Calculate total from order part add-ons
        const partAddOnsTotal = currentOrderPart?.orderPartSelectedOptions?.reduce((total, option) => {
            if (option.linkType === 'ADDON') {
    ...
    const calculateTotalQuantity = () => {
        const total = currentOrderPart?.orderPartFiles?.reduce((total, file) => {
    ...
    const handleDiscountChange = (value) => {
        ...
        setOrderParts(prev => prev.map(part =>
            part.id === activeTab
                ? {
                    ...part,
    ...
    useEffect(() => {
        ...
        const calculateTotalPrices = () => {
            ...
            const updatedFiles = currentOrderPart.orderPartFiles?.map(file => {
            ...
            const filesTotal = updatedFiles.reduce((sum, file) =>
                sum + parseFloat(file.finalFilePrice || 0), 0);

            // Calculate add-ons total
            const totalQuantity = calculateTotalQuantity();
            const addOnsTotal = currentOrderPart.orderPartSelectedOptions?.reduce((total, option) => {
                if (option.linkType === 'ADDON') {
                    ...
                    {currentOrderPart?.orderPartFiles?.map((file) => (
                        <Grid container key={file.orderPartFile.fileId} sx={{ py: 1 }} style={{ paddingLeft: "10px" }}>
                            ...
                            <Grid item xs={5.5}>
                                ...
                                }}>{file.orderPartFile.fileName}</Typography>
                            </Grid>

                            <Grid item xs={2}>
                                <Typography sx={{ fontSize: '14px' }}>
                                    {calculateFileQuantity(file)}
                                    {!service ? "" : service?.unit === "perMinute" || service?.unit === "perSecond"
                                        ? ' m' : ` ${service?.unit}`}
                                </Typography>
                            </Grid>
                            <Grid item xs={2}>
                ...
                <>
                    ...
                    {currentOrderPart?.orderPartSelectedOptions?.map((option) => (
                        <Grid container key={option.linkId} sx={{ py: 1 }} style={{ paddingLeft: "10px" }}>
                            <Grid item xs={5.5}>
                                <Typography sx={{ fontSize: '14px' }}>{addOns?.find(addOn => addOn?.addonId === option?.linkId)?.addonName}, {option.linkName}</Typography>
                            </Grid>
                            {/* <Grid item xs={3}>
                ...
                <Grid item xs={4}>
                    <Box sx={{
                        ...
                        <TextField
                            ...
                            error={discountType === 'percentage' && parseFloat(currentOrderPart?.manualDiscount?.replace('%', '') || 0) > 100}
                            helperText={
                                discountType === 'percentage' &&
