...
import { Box, Typography, Grid, TextField, Select, MenuItem, InputAdornment } from '@mui/material';
...
const PriceCalculation = ({
    ...
    const [discountType, setDiscountType] = useState(() => {
        const currentFile = orderParts
            ...
            ?.orderPartFiles
            ?.find(f => f.orderPartFile?.fileId === file?.fileId || f.orderPartFile?.fileId === file?.file?.fileId);
    ...
    const calculateFileQuantity = useCallback(() => {
        ...
        if ((service?.unit !== "perMinute" && service?.unit !== "perSecond") && service?.isFiles === true) {
            const quantity = currentFile?.unitQuantities?.length > 0
    ...
    const calculateDiscount = useCallback((value) => {
        ...
        return (currentFile?.fileSelectedOptions || []).map((option) => {
            ...
            return (
                <Grid container key={option.linkId} sx={{ py: 1 }} style={{ paddingLeft: "10px" }}>
                    ...
                    <Grid item xs={2}>
                        <Typography sx={{ fontSize: '14px' }}>{quantity} {option?.unitName === "perMinute" || option?.unitName === "perSecond" ? ' m' : ` ${option?.unitName}`}</Typography>
                    </Grid>
                    <Grid item xs={2}>
            ...
            {!service ?
                ...
                <>
                    ...
                    <Grid container sx={{ py: 1 }} style={{ paddingLeft: "10px" }}>
                        ...
                        <Grid item xs={2}>
                            <Typography sx={{ fontSize: '14px' }}>
                                {parseFloat(calculateFileQuantity() || 0).toFixed(2)} {!service ? "" : service?.unit === "perMinute" || service?.unit === "perSecond" ? ' m' : ` ${service?.unit}`}
                            </Typography>
                        </Grid>
                        <Grid item xs={2}>
                ...
                <Grid item xs={4}>
                    <Box sx={{
                        ...
                        <TextField
                            ...
                            error={discountType === 'percentage' && parseFloat(currentFile?.manualDiscount?.replace('%', '') || 0) > 100}
                            helperText={
                                discountType === 'percentage' &&
                                    parseFloat(currentFile?.manualDiscount?.replace('%', '') || 0) > 100
